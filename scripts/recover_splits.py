...
def main():
    ...
    for run_dir, seed in runs:
        if not os.path.isdir(run_dir):
            # Counted, not waved through: a directory that is not there was never checked,
            # and reporting it as verified would invert the point of this script.
    ...
    print()
    if args.check:
        print(f"verified {verified} of {len(runs)} runs, wrote nothing"
              + (f", {missing} not found" if missing else ""))
    else:
        print(f"wrote {written} split files, skipped {skipped} existing, "
              f"{failures} runs failed" + (f", {missing} not found" if missing else ""))
    # A run that could not be found is not a success: exit non-zero so a broken path in a
    # sweep cannot be mistaken for a clean verification.
