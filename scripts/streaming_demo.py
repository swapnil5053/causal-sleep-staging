...
def main():
    ...
    if args.mode == "window":
        for window_start in range(start, last, context):
            if window_start + context > len(x):
                break
            take = min(context, last - window_start)
            stager.set_context(x[window_start:window_start + context])
        ...
        for second in range(start, last):
            stager.set_context(x[second - context + 1:second + 1])
            began = time.perf_counter()
    ...
    if args.verify:
        ...
        streamed = []
        for window_start in range(0, len(x) - context + 1, context):
            stager.set_context(x[window_start:window_start + context])
        ...
        identical = np.array_equal(streamed, reference)
        mismatches = int((streamed != reference).sum())
        print(f"  seconds compared : {len(streamed):,}")
        print(f"  mismatches       : {mismatches}")
        print(f"  result           : {'PASS - identical' if identical else 'FAIL'}")
        if not identical:
            status = 1
        else:
            print("  The streaming front end reproduces the archived evaluation exactly, so the")
            print("  reported kappa is the number this device path would produce.")
